...
// $filekey = json_decode(file_get_contents('key.json'), true);
...
//     $SKey = $_POST['key'];
// } else {
//     echo json_encode(array("Status" => "Error", "msg" => "Bạn chưa nhập Key!"), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
//     exit;
// }

// if (!array_key_exists($SKey, $filekey)) {
//     echo json_encode(array("Status" => "Error", "msg" => "Key sai hoặc không tồn tại!"), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
//     exit;
// }
...
// 	$sdt = $_POST["phone"];
// } else {
// 	echo json_encode(array("Status" => "Error", "msg" => "Hãy nhập sđt!"), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT); exit;
// }
// if(strlen($sdt) < 10){
// 	$array = array(
//     "Status" => "Error",
// 		"msg" => "Bạn không nhập đúng định dạng!"
// 	);
// 	echo json_encode($array, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT); exit;
// } else if(!$sdt){
// 	$array = array(
// 		"Status" => "Error",
// 		"msg" => "Bạn chưa nhập sđt!"
// 	);
// 	echo json_encode($array, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT); exit;
// }
// if ($filekey[$SKey] <= 0) {
//     echo json_encode(array("Status" => "Error", "msg" => "Key đã hết lượt dùng!"), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
//     exit;
// }

// $filekey[$SKey]--;

// file_put_contents('key.json', json_encode($filekey, JSON_PRETTY_PRINT));
...
$array = array(
	"01.MONEYVEO" => $MONEYVEO["MONEYVEO"],
	"02.ROBOCASH" => $ROBOCASH["ROBOCASH"],
	"03.DONGPLUS" => $DONGPLUS["DONGPLUS"],
	"04.TIENOI" => $TIENOI["TIENOI"],
	"05.MOMO" => $MOMO["MOMO"],
	"06.ITAPHOA" => $ITAPHOA["ITAPHOA"],
	#===============
	"07.VAYVND" => $VAYVND["VAYVND"],
	"08.LOSHIP" => $LOSHIP["LOSHIP"],
	"09.TUOITRE" => $TUOITRE["TUOITRE"],
	"10.VIEON" => $VIEON["VIEON"],
	"11.F88" => $F88["F88"],
	"12.TAMO" => $TAMO["TAMO"],
	"13.VIETTELL" => $VIETTELL["VIETTELL"],
	"14.VT2" => $VETTELL2["VETTELL2"],
	"15.ZALOPAY" => $ZALOPAY["ZALOPAY"],
	"16.WINMART" => $WINMART["WINMART"],
	"17.META" => $META["META"],
	"18.FPTSHOP" => $FPTSHOP["FPTSHOP"],
	"19.TV360" => $TV360["TV360"],
	"20.CAYDENTHAN" => $CAYDENTHAN["DENTHAN"],
	"21.ONCREDIT" => $ONCREDIT["ONCREDIT"],
	"22.DAIHOCFPT" => $DAIHOCFPT["DAIHOCFPT"],
	"23.CAFELAND" => $CAFELAND["CAFELAND"],
	"24.VIETTELLPAY" => $VTPAY["VTPAY"],
	"25.VAYSIEUDE" => $VAYSIEUDE["VAYSIEUDE"],
	"27.OPS" => $POPS["POPS"],
	"28.CONCUNG" => $CONCUNG["CONCUNG"],
	"29.THANTAIOI" => $THANTAIOI["THANTAIOI"],
	"30.UBO" => $UBO["UBO"],
	"31.SAPO" => $SAPO["SAPO"],
	"32.POPEYS" => $POPEYS["POPEYS"],
	"33.PHARMACITY" => $PHARMACITY["PHARMACITY"],
	"34.MIOTO" => $MIOTO["MIOTO"],
	"35.KILO" => $KILO["KILO"],
	"36.SRC8BET" => $SRC8BET["SRC8BET"],
	"37.FPT" => $FPT["FPT"],
	"38.VAMO" => $VAMO["VAMO"],
	"39.SENMO" => $SENMO["SENMO"],
	"40.INTERLOAD" => $INTERLOAD["INTERLOAD"]
);
$data = array(
	"status" => "Done",
	"DATA" => $array,
...
function MONEYVEO($sdt){
	...
	if($access["succeed"] == "true" or $_GET["succeed"] == 1){
